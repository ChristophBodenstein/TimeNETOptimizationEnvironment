...
public class DownloadXMLService {
	...
	public Response getPreview(@PathParam("flag") String flag) {
		...
		try{
			...
			response = Response.ok((Object) std.getFileData());
			...
			LogFile.info("--------Simulation XML file download to generate log file---------"+std.getSimulation_file_name());
			...
			LogFile.debug("No XML-File in List for Downloads (get(0) caused Exception).");
