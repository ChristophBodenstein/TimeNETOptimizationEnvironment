...
public class StatisticAggregator {
    ...
    public static void printOptiStatistics() {
        ...
        if (numberOfAllOptimizationRuns >= 1) {
            ...
            support.log(p.getPref_WrongSimulationsUntilBreak() + "   & " + p.getPref_SizeOfNeighborhood() + "\\%     &" + p.getPref_WrongSimulationsPerDirection() + "      &" + support.round(averageDistanceToOptimumInValueRange) + "\\%     &" + support.round(averageDistanceToOptimumInDefinitionRange) + "\\%    &" + averageNumberOfSimulations + "    &" + averageCPUTimeTotal + "     &" + support.round(averageCacheRatio) * 100 + "\\%  \\\\  \\hline", typeOfLogLevel.RESULT);
